...
single_positive_cls_labels_dict = np.load('coco14/single_positive_cls_labels_coco.npy', allow_pickle=True).item()
...
def load_single_positive_label_list_from_npy(img_name_list):

    return np.array([single_positive_cls_labels_dict[img_name] for img_name in img_name_list])
...
class COCO14SinglePositiveClassificationDataset(COCO14ImageDataset):
    ...
    def __init__(self, img_name_list_path, coco14_root,
        ...
        super().__init__(img_name_list_path, coco14_root,
                 resize_long, rescale, img_normal, hor_flip,
                 crop_size, crop_method)
        self.label_list = load_single_positive_label_list_from_npy(self.img_name_list)
