...
if __name__ == '__main__':
    ...
    parser.add_argument("--amn_weights_name", default="res50_amn_coco.pth", type=str)
    parser.add_argument("--amn_cam_out_dir", default="amn_cam", type=str)
    parser.add_argument("--amn_ir_label_out_dir", default="amn_ir_label",
                        type=str)
    ...
    parser.add_argument("--amn_conf_fg_thres", default=0.45, type=float) # 0.35
    parser.add_argument("--amn_conf_bg_thres", default=0.15, type=float) # 0.15
    ...
    parser.add_argument("--cam_weights_name", default="res50_cam_coco.pth", type=str)
    parser.add_argument("--irn_weights_name", default="res50_irn_coco.pth", type=str)

    parser.add_argument("--cam_out_dir", default="cam_coco", type=str)

    parser.add_argument("--ir_label_out_dir", default="ir_label",
                        type=str)
    ...
    parser.add_argument("--make_sem_seg_pass", type=str2bool, default=True)  # check power
    ...
    if args.train_cam_pass is True:
        import step_coco.train_cam_coco

        timer = pyutils.Timer('step_coco.train_cam:')
        step_coco.train_cam_coco.run(args)

    if args.make_cls_labels_pass is True:
        import step_coco.make_cls_labels

        timer = pyutils.Timer('step.make_cls_labels:')
        step_coco.make_cls_labels.run(args)

    if args.make_cam_pass is True:
        import step_coco.make_cam_coco

        timer = pyutils.Timer('step_coco.make_cam:')
        step_coco.make_cam_coco.run(args)

    if args.eval_cam_pass is True:
        import step_coco.eval_cam_coco

        timer = pyutils.Timer('step_coco.eval_cam:')
        final_miou = []
        # for i in range(10, 51):
        # i = 15
        # t = i/100.0
        # args.cam_eval_thres = t
        miou = step_coco.eval_cam_coco.run(args)
        final_miou.append(miou)
        print(args.cam_out_dir)
        print(final_miou)
        print(np.max(np.array(final_miou)))
    if args.cam_to_ir_label_pass is True:
        import step_coco.cam_to_ir_label_coco

        timer = pyutils.Timer('step_coco.cam_to_ir_label:')
        step_coco.cam_to_ir_label_coco.run(args)

    if args.train_amn_pass is True:
        import step_coco.train_amn_coco

        timer = pyutils.Timer('step_coco.train_le:')
        step_coco.train_amn_coco.run(args)

    if args.make_amn_cam_pass is True:
        import step_coco.make_amn_cam_coco

        timer = pyutils.Timer('step_coco.make_amn_cam:')
        step_coco.make_amn_cam_coco.run(args)

    if args.eval_amn_cam_pass is True:
        import step_coco.eval_amn_cam_coco
        ...
        # t = i/100.0
        # args.cam_eval_thres = t
        miou = step_coco.eval_amn_cam_coco.run(args)
        final_miou.append(miou)
        print(args.cam_out_dir)
        print(final_miou)
        print(np.max(np.array(final_miou)))
    ...
    if args.amn_cam_to_ir_label_pass is True:
        import step_coco.amn_cam_to_ir_label_coco

        timer = pyutils.Timer('step_coco.cam_to_ir_label:')
        step_coco.amn_cam_to_ir_label_coco.run(args)

    if args.train_irn_pass is True:
        args.num_workers = 0
        import step_coco.train_irn_coco

        timer = pyutils.Timer('step_coco.train_irn:')
        step_coco.train_irn_coco.run(args)

    if args.make_sem_seg_pass is True:
        import step_coco.make_sem_seg_labels_coco
        args.sem_seg_bg_thres = float(args.sem_seg_bg_thres)
        timer = pyutils.Timer('step_coco.make_sem_seg_labels:')
        step_coco.make_sem_seg_labels_coco.run(args)

    if args.eval_sem_seg_pass is True:
        import step_coco.eval_sem_seg_coco

        timer = pyutils.Timer('step_coco.eval_sem_seg:')
        step_coco.eval_sem_seg_coco.run(args)
