...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            if os.path.exists(os.path.join(args.amn_cam_out_dir, img_name + '.npy')):
                continue
...
def run(args):
    model = getattr(importlib.import_module(args.amn_network), 'CAM')(coco=True)
    ...
    dataset.label_list = np.load(os.path.join(args.work_space, 'activated_class_labels.npy'))
